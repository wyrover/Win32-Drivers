...
szAbout				db "About...", 0
szWrittenBy			db "Interrupt Descriptor Table Dumper v1.0", 0Ah, 0Dh
					db "Compiled on "
					date
					db 0Ah, 0Dh, 0Ah, 0Dh
...
local dwVector:DWORD
	...
	invoke lstrcat, edi, $CTA0("Int.  Sel.:Offset    DPL  P   D        Description\n")
	invoke lstrcat, edi, addr g_szLine
	...
	.while ebx
		...
		.if ecx == 0101y
			; Task Gate
			invoke lstrcat, edi, $CTA0("Task Gate")
		.elseif ecx == 0110y
			; Interrupt Gate
			invoke lstrcat, edi, $CTA0("Interrupt Gate")
		.elseif ecx == 0111y
			; Trap Gate
			invoke lstrcat, edi, $CTA0("Trap Gate")
		.endif
